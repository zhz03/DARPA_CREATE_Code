...
def verify_Usample_H01():
    ...
    uts = [0,1]
    ts = [100,1]
    Uz_H0,Uz_H1 = Stat_Uz_H01(A,B,H,Q,R,x0,uts,ts[0],sample_num)
    Uz_H1 = cnvdata.convert_array2list_1d(Uz_H1)
    Uz_H0 = cnvdata.convert_array2list_1d(Uz_H0)
...
def verify_Usample():
    ...
    uts = [0,1]
    ts = [100,1]
    Uz_H1 = Stat_Uz(A,B,H,Q,R,x0,uts,ts,sample_num)
    Uz_H1 = cnvdata.convert_array2list_1d(Uz_H1)
    samples = Uz_H1
    # ---------------------
    # planning USigma and Umean from KF planning
...
#========================The following are some old codes=========================================== 
...
def E2E_stats_error_prob(A,B,H,Q,R,x0,uts,ts,sam_numb,flag = 1):
    ...
    #timesteps = np.sum(ts)
    if flag == 1:
        Uz_H0,Uz_H1 = Stat_Uz_H01(A,B,H,Q,R,x0,uts,ts[0],sam_numb)
    elif flag == 2:
        Uz_H1 = Stat_Uz(A,B,H,Q,R,x0,uts,ts,sam_numb)
        uts0 = [0]
        ts0 = [np.sum(ts)]
        Uz_H0 = Stat_Uz(A,B,H,Q,R,x0,uts0,ts0,sam_numb)
